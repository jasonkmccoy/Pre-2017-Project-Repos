...
var webApp = function() {
    ...
    var uiInit = function() {
        ...
        pageContent.css('min-height', $(window).height() - ($('header').height() + parseInt(pageContent.css('padding-top')) * 2 + $('footer').outerHeight()) + 'px');
        ...
        $.extend($.fn.dataTableExt.oPagination, {
            "bootstrap": {
                "fnInit": function(oSettings, nPaging, fnDraw) {
                    ...
                    $(nPaging).addClass('pagination').append('<ul>' + '<li class="prev disabled"><a href="javascript:void(0)"><i class="icon-chevron-left"></i> ' + oLang.sPrevious + '</a></li>' + '<li class="next disabled"><a href="javascript:void(0)">' + oLang.sNext + ' <i class="icon-chevron-right"></i></a></li>' + '</ul>');
                    var els = $('a', nPaging);
                    $(els[0]).bind('click.DT', {
                        action: "previous"
                    }, fnClickHandler);
    ...
    return {
        ...
        init: function() {
            ...
            // webApp_pdfLinks(); // Initialize PDF Icon function
            webApp_expiredLinks(); // Initialize Expired Icon function
        }
    };
}();

function webApp_pdfLinks() {
    var links = document.getElementsByTagName('a');
    for (var i = 0; i < links.length; i++) {
        var link = links[i];
        if (link.text.indexOf('pdf') > -1 || link.href.indexOf('pdf') > -1) {
            var pdfImage = document.createElement('img');
            pdfImage.src = 'http://domco.us/files/img/pdf.gif';
            pdfImage.style['margin-left'] = "5px";
...
function webApp_expiredLinks() {
    var currentDate = new Date();
    var expiredLinks = document.getElementsByClassName('expires');
    for (var i = 0; i < expiredLinks.length; i++) {
        var link = expiredLinks[i];
        var parentNode = expiredLinks[i].parentNode;
        var expires = new Date(link.getAttribute('data-expires'));
        if (!isNaN(expires)) {
            var img = document.createElement('img');
            if (currentDate < expires) {
                img.className = 'imgGood';
                img.src = 'http://domco.us/files/img/good.png';
            } else if (currentDate > expires) {
                img.className = 'imgBad';
                img.src = 'http://domco.us/files/img/bad.png';
            }
